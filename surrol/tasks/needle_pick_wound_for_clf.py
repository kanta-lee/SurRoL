...
class NeedlePickWoundCLF(PsmEnv):
    ...
    def _env_setup(self):
        ...
        self.cyl_radius = 0.05
        cyl_length = 0.1

        # offset the cylinder slightly in -x to better cover the goal region
        self.cylinder_offset = np.array([-0.02, 0.0, 0.0])
        cyl_pos = (
            workspace_limits[0].mean(),
            workspace_limits[1].mean(),
            workspace_limits[2][0] + 0.045
        )
        self.cylinder_center = np.array(cyl_pos, dtype=float)
    ...
    def _sample_goal_callback(self):
        ...
        # set cylinder pose around goal
        self.cylinder_center = np.array([self.goal[0], self.goal[1], self.goal[2]]) + self.cylinder_offset
