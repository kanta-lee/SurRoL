...
class GauzeRetrieve(PsmEnv):
    ...
    def _env_setup(self):
        ...
        p.changeVisualShape(obj_id, -1, rgbaColor=(225 /
                            255, 225 / 255, 225 / 255, 1))
        ...
        obstacle_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'box/surface_obstacle_col.urdf'),
                                 globalScaling=self.SCALING)
    ...
    def _sample_goal_callback(self):
        ...
        # Reset surface obstacle position (constant so far)
        p.resetBasePositionAndOrientation(
            self.obj_ids['obstacle'][0],
            np.array([2.7, 0.2, 3.5]),
            (-0.5, 0., 0., 0.8660254))
