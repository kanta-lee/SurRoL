...
class BiPegBoard(PsmsEnv):
    ...
    def _env_setup(self):
        ...
        # board
        asset_scaling = 0.185
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, "peg_board/ring_board.urdf"),
        ...
        # peg 
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, "peg_board/ring_peg.urdf"),
                            np.array(self.POSE_BOARD[0]) * self.SCALING,
                            p.getQuaternionFromEuler(self.POSE_BOARD[1]),
                            globalScaling=asset_scaling,
                            useFixedBase=1)
        ...
        for i in self._pegs[2: 2 + num_rings]:
            ...
            obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, "ring/ring.urdf"),
